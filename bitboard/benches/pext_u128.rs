...
impl BB {
    #[inline]
    fn pext_bmi2(&self, mask: &Self) -> u128 {
    ...
    #[inline]
    fn pext_fallback(&self, mask: &Self) -> u128 {
        let mut res: u128 = 0;
        let mut bit: u128 = 1;
        let mut m = mask.bits;
        let mut v = self.bits;
...
fn bench_pext_u128(c: &mut Criterion) {
    let mut group = c.benchmark_group("pext_u128");
...
criterion_group!(benches, bench_pext_u128);
